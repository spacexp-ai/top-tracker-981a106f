  ...
  head: () => ({
    meta: [
      { title: "Book a Safari — Top Trackers" },
      {
        name: "description",
        content:
          "Begin your African hunting expedition. Live weather, interactive map, and secure deposit checkout.",
      },
    ],
  }),
  component: Contact,
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[50svh] bg-ink overflow-hidden">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-center px-6 text-bone">
          <Eyebrow light>
            {getContent("contact.hero.eyebrow", t("contact.hero.eyebrow", "Plan your expedition"))}
          </Eyebrow>
          <h1 className="mt-6 font-display text-5xl md:text-7xl max-w-4xl leading-[0.95] text-bone">
            ...
            <span className="italic font-serif text-accent">
              {getContent("contact.hero.title_italic", t("contact.hero.title_italic", "safari."))}
            </span>
          </h1>
        </div>
      </section>
          ...
          <Reveal className="lg:col-span-2">
            ...
            >
              <div className="text-[10px] tracking-[0.4em] uppercase text-accent mb-2">
                {getContent("contact.form.eyebrow", t("contact.form.eyebrow", "Inquiry"))}
              </div>
              <h3 className="font-display text-2xl text-forest mb-8">
                {getContent(
                  "contact.form.title",
                  t("contact.form.title", "Tell us what you seek."),
                )}
              </h3>
              ...
              {sent ? (
                <div className="py-16 text-center">
                  <div className="font-display text-3xl text-ember">
                    {getContent(
                      ...
                      t("contact.form.success_title", "Karibu."),
                    )}
                  </div>
                  <p className="mt-3 font-serif text-lg text-foreground/75">
                    {getContent(
                      ...
                      t(
                        "contact.form.success_body",
                        "Your message is on its way to camp. We'll be in touch shortly.",
                      ),
                    )}
                  </p>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <Field
                    label={t("contact.form.country", "Country")}
                    name="country"
                    value={country}
                    onChange={setCountry}
                  />
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-2">
                        {t("contact.form.date", "Preferred start date")}
                      </label>
                      <Popover>
                    ...
                    <div>
                      ...
                      >
                        <option value="The Selous Classic">
                          {t("contact.form.interest_option1", "The Selous Classic")}
                        </option>
                        <option value="Maasai Steppe Plains">
                          {t("contact.form.interest_option2", "Maasai Steppe Plains")}
                        </option>
                        <option value="Iringa Highlands">
                          {t("contact.form.interest_option3", "Iringa Highlands")}
                        </option>
                        <option value="Club Membership">
                          {t("contact.form.interest_option4", "Club Membership")}
                        </option>
                  ...
                  <div>
                    <label className="block text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-2">
                      {t("contact.form.more", "Tell us more")}
                    </label>
                    <textarea
                      rows={5}
                      maxLength={2000}
                      ...
                      placeholder={t(
                        ...
                        "Quarry, party size, anything we should know…",
                      )}
                    />
                  </div>
                    ...
                    >
                      {payState === "loading" ? (
                        <>
                          ...
                          {t("contact.form.preparing", "Preparing…")}
                        </>
                      ) : (
                        <>
                          <CreditCard className="h-4 w-4" />{" "}
                          {t("contact.form.reserve", "Reserve with $1500 deposit")}
                        </>
                      )}
                    </button>
                  </div>
            ...
            <Reveal delay={0.3}>
              <div className="bg-card border border-border p-6">
                <div className="text-[10px] tracking-[0.4em] uppercase text-accent mb-3">
                  {getContent("contact.office.title", t("contact.office.title", "Office"))}
                </div>
                <ul className="space-y-3 font-serif text-base">
                  <li className="flex gap-3">
                    <MapPin className="h-4 w-4 text-ember mt-1 shrink-0" />{" "}
                    {getContent(
                      "contact.office.address",
                      t("contact.office.address", "Losirva, Esilalei, Monduli — Tanzania"),
                    )}
                  </li>
                  <li className="flex gap-3">
                    <Phone className="h-4 w-4 text-ember mt-1 shrink-0" />{" "}
                    {getContent(
                      "contact.office.phone",
                      t("contact.office.phone", "+255 763 075 130"),
                    )}
                  </li>
                  <li className="flex gap-3">
                    <Mail className="h-4 w-4 text-ember mt-1 shrink-0" />{" "}
                    {getContent(
                      "contact.office.email",
                      t("contact.office.email", "info@top-trackers.com"),
                    )}
                  </li>
                </ul>
                <div className="mt-5 pt-4 border-t border-border">
                  <div className="text-[10px] tracking-[0.4em] uppercase text-accent mb-2">
                    {getContent("contact.camp.title", t("contact.camp.title", "Base Camp"))}
                  </div>
                  <div className="font-serif text-sm text-foreground/75">
                    {getContent(
                      "contact.camp.body",
                      t("contact.camp.body", "on the bank of Njombe River"),
                    )}
                  </div>
                  <div className="mt-1 text-[11px] tracking-[0.18em] text-muted-foreground">
                    {getContent(
                      "contact.camp.coordinates",
                      t("contact.camp.coordinates", "6°54′21.0″S · 34°59′14.8″E"),
                    )}
                  </div>
                </div>
              </div>
