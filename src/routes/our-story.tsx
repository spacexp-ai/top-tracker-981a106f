  ...
  head: () => ({
    meta: [
      { title: "Our Story — Top Trackers" },
      {
        name: "description",
        content:
          "More than a hunt — a legacy. The story, ethics, and conservation heritage behind Top Trackers, in the heart of Tanzania.",
      },
      { property: "og:title", content: "Our Story — Top Trackers" },
      {
        property: "og:description",
        content: "The legacy and ethics behind Africa's premier hunting club.",
      },
  ...
  const timeline = [
    {
      ...
      body: getContent(
        "ourstory.timeline.1.body",
        t(
          "ourstory.timeline.1.body",
          "Top Trackers is founded by a small circle of Tanzanian PHs and European hunters seeking a quieter, more ethical way to hunt.",
        ),
      ),
    },
    {
      ...
      title: getContent(
        "ourstory.timeline.2.title",
        t("ourstory.timeline.2.title", "The concessions"),
      ),
      image: resolveImage(getContent("ourstory.timeline.2.image", "vintageLeopard")),
      body: getContent(
        "ourstory.timeline.2.body",
        t(
          "ourstory.timeline.2.body",
          "Long-term stewardship agreements signed across three legendary concessions in the Selous, Maasai Steppe and Iringa highlands.",
        ),
      ),
    },
    {
      ...
      body: getContent(
        "ourstory.timeline.3.body",
        t(
          "ourstory.timeline.3.body",
          "Our Base camp opens — canvas, brass, and lantern light, built with local Maasai craftsmen.",
        ),
      ),
    },
    {
      ...
      body: getContent(
        "ourstory.timeline.4.body",
        t(
          "ourstory.timeline.4.body",
          "Membership opens to a limited circle of trackers, professional hunters, and legacy patrons.",
        ),
      ),
    },
  ];
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[75svh] bg-ink overflow-hidden">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-center px-6 text-bone">
          <Eyebrow light>
            {getContent("ourstory.hero.eyebrow", t("ourstory.hero.eyebrow", "Our story"))}
          </Eyebrow>
          <h1 className="mt-8 font-display text-5xl md:text-8xl max-w-5xl leading-[0.95]">
            {getContent(
              "ourstory.hero.title_line1",
              t("ourstory.hero.title_line1", "More than a hunt."),
            )}
            <br />
            <span className="italic font-serif text-accent">
              {getContent("ourstory.hero.title_line2", t("ourstory.hero.title_line2", "A legacy."))}
            </span>
          </h1>
          <p className="mt-8 font-serif text-2xl text-bone/80 max-w-2xl">
            {getContent(
              "ourstory.hero.subtitle",
              t(
                "ourstory.hero.subtitle",
      ...
      <section className="bg-ink text-bone py-32 relative grain">
        <div className="relative mx-auto max-w-5xl px-6">
          <Reveal>
            <Eyebrow light>
              {getContent(
                "ourstory.timeline.eyebrow",
                t("ourstory.timeline.eyebrow", "The trail so far"),
              )}
            </Eyebrow>
          </Reveal>
          <Reveal delay={0.1}>
            <h2 className="mt-6 font-display text-4xl md:text-5xl mb-16">
              {getContent(
                "ourstory.timeline.title",
                t("ourstory.timeline.title", "A short, deliberate history."),
              )}
            </h2>
          </Reveal>
          <div className="relative pl-8 md:pl-0">
            ...
            {timeline.map((t, i) => (
              <Reveal key={t.year} delay={i * 0.08}>
                <div
                  className={`group relative mb-24 md:grid md:grid-cols-2 gap-12 ${i % 2 ? "md:[&>*:first-child]:order-2" : ""}`}
                >
                  <div className={`md:text-right ${i % 2 ? "md:text-left" : ""}`}>
                    <div className="font-display text-5xl text-accent transition-colors duration-500 group-hover:text-ember">
                      {t.year}
                    </div>
                  </div>
                  <div className="relative">
                    <h3 className="font-display text-2xl tracking-[0.15em] uppercase">{t.title}</h3>
                    <p className="mt-3 font-serif text-lg text-bone/75 transition-colors duration-500 group-hover:text-bone">
                      {t.body}
                    </p>

                    {/* Hover Image */}
                    <div
                      className={`absolute top-full mt-8 w-full max-w-md aspect-[4/3] overflow-hidden opacity-0 scale-95 pointer-events-none group-hover:opacity-100 group-hover:scale-100 transition-all duration-700 z-10 hidden md:block ${i % 2 ? "right-0 origin-top-right" : "left-0 origin-top-left"}`}
                    >
                      <img
                        src={t.image}
                        alt={t.year}
        ...
        <div className="max-w-3xl mx-auto px-6">
          <Reveal>
            <h2 className="font-display text-4xl md:text-5xl text-forest">
              {getContent(
                "ourstory.footer.title",
                t("ourstory.footer.title", "Walk the trail with us."),
              )}
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            ...
            >
              {t("ourstory.footer.explore", "Explore Membership")}{" "}
