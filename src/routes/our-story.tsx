...
function OurStory() {
  ...
  const timeline = [
    {
      year: getContent("ourstory.timeline.1.year", "2023"),
      title: getContent("ourstory.timeline.1.title", "First fire"),
      image: resolveImage(getContent("ourstory.timeline.1.image", "timeline2023")),
      body: getContent("ourstory.timeline.1.body", "Top Trackers is founded by a small circle of Tanzanian PHs and European hunters seeking a quieter, more ethical way to hunt."),
    },
    {
      year: getContent("ourstory.timeline.2.year", "2024"),
      title: getContent("ourstory.timeline.2.title", "The concessions"),
      image: resolveImage(getContent("ourstory.timeline.2.image", "vintageLeopard")),
      body: getContent("ourstory.timeline.2.body", "Long-term stewardship agreements signed across three legendary concessions in the Selous, Maasai Steppe and Iringa highlands."),
    },
    {
      year: getContent("ourstory.timeline.3.year", "2025"),
      title: getContent("ourstory.timeline.3.title", "The camp"),
      image: resolveImage(getContent("ourstory.timeline.3.image", "campNight")),
      body: getContent("ourstory.timeline.3.body", "Our Base camp opens — canvas, brass, and lantern light, built with local Maasai craftsmen."),
    },
    {
      year: getContent("ourstory.timeline.4.year", "2026"),
      title: getContent("ourstory.timeline.4.title", "The club"),
      image: resolveImage(getContent("ourstory.timeline.4.image", "phWalking")),
      body: getContent("ourstory.timeline.4.body", "Membership opens to a limited circle of trackers, professional hunters, and legacy patrons."),
    },
  ];
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[75svh] bg-ink overflow-hidden">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-center px-6 text-bone">
          <Eyebrow light>{getContent("ourstory.hero.eyebrow", "Our story")}</Eyebrow>
          <h1 className="mt-8 font-display text-5xl md:text-8xl max-w-5xl leading-[0.95]">
            {getContent("ourstory.hero.title_line1", "More than a hunt.")}<br />
            <span className="italic font-serif text-accent">{getContent("ourstory.hero.title_line2", "A legacy.")}</span>
          </h1>
          <p className="mt-8 font-serif text-2xl text-bone/80 max-w-2xl">
            {getContent("ourstory.hero.subtitle", "The ethics of modern safari hunting, written in patience.")}
          </p>
        </div>
      </section>
        ...
        <div className="relative mx-auto max-w-3xl px-6 font-serif text-xl leading-relaxed text-foreground/85 space-y-8">
          <Reveal>
            <p className="text-3xl text-forest font-display tracking-wide">
              {getContent("ourstory.intro.highlight", "We aim to provide world-class hunting experiences that directly contribute to wildlife conservation and local community development.")}
            </p>
          </Reveal>
          <Reveal delay={0.1}>
            <p>
          ...
          <Reveal delay={0.2}>
            <p>
              {getContent("ourstory.intro.body2", "Modern hunting, done well, is not the opposite of conservation — it is one of its most powerful instruments. A percentage of every safari we run is returned to the land: anti-poaching units, water boreholes, school roofs, and game counts.")}
            </p>
          </Reveal>
          <Reveal delay={0.3}>
            <p className="italic text-ember">
              {getContent("ourstory.intro.callout", "This is the Top Tracker's way. Slow, considered, and quietly proud.")}
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="bg-ink text-bone py-32 relative grain">
        <div className="relative mx-auto max-w-5xl px-6">
          <Reveal><Eyebrow light>{getContent("ourstory.timeline.eyebrow", "The trail so far")}</Eyebrow></Reveal>
          <Reveal delay={0.1}>
            <h2 className="mt-6 font-display text-4xl md:text-5xl mb-16">
              {getContent("ourstory.timeline.title", "A short, deliberate history.")}
            </h2>
          </Reveal>
        ...
        <div className="max-w-3xl mx-auto px-6">
          <Reveal>
            <h2 className="font-display text-4xl md:text-5xl text-forest">
              {getContent("ourstory.footer.title", "Walk the trail with us.")}
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
