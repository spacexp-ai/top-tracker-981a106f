  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[50svh] bg-ink overflow-hidden">
        <img src={campImage} alt="Partners banner" className="absolute inset-0 w-full h-full object-cover opacity-55" />
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-center px-6 text-bone">
          ...
          <h1 className="mt-6 font-display text-5xl md:text-7xl max-w-4xl leading-[0.95]">
            <span className="italic font-serif text-accent">{getContent("partners.hero.title_italic", "Partners")}</span> {getContent("partners.hero.title_plain", "in the field.")}
          </h1>
        </div>
      </section>
      ...
      <section className="bg-ink text-bone py-24">
        <div className="mx-auto max-w-3xl px-6">
          <Reveal><Eyebrow light>{getContent("partners.form.eyebrow", "Open the conversation")}</Eyebrow></Reveal>
          <h2 className="mt-6 font-display text-3xl md:text-4xl">{getContent("partners.form.title", "Tell us where you fit.")}</h2>

          {sent ? (
            <div className="mt-10 p-10 border border-accent/40 text-center">
              <div className="font-display text-3xl text-accent">{getContent("partners.form.success_title", "Asante.")}</div>
              <p className="mt-3 font-serif text-bone/75">{getContent("partners.form.success_body", "We'll be in touch within five business days.")}</p>
            </div>
          ) : (
            <form
