...
import { ArrowLeft, MapPin, Calendar, ExternalLink, Shield, MessageSquare, Trophy } from "lucide-react";
...
function TrackerProfile() {
    ...
    return (
      <PortalShell title="Tracker Profile">
        <div className="p-8 text-[#a8a8a0]">
          ...
          <div className="mt-4">
            <Link to="/portal/community" className="text-[#c9a84c] text-xs tracking-[0.3em] uppercase hover:underline">← Back to Lodge</Link>
          </div>
        </div>
      </PortalShell>
    );
  ...
  return (
    <PortalShell title={`${profile.name} — Field Record`}>
      <div className="mb-6">
        <Link to="/portal/community" className="inline-flex items-center gap-2 text-[10px] tracking-[0.3em] uppercase text-[#a8a8a0] hover:text-[#c9a84c] transition">
        ...
        <div className="px-6 md:px-10 pb-8 relative">
          <div className="flex flex-col md:flex-row md:items-end gap-6 -mt-16 md:-mt-20 relative z-10">
            ...
            <div className="flex-grow pb-2">
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3 text-xs tracking-[0.1em] uppercase text-[#a8a8a0]">
                <span className="flex items-center gap-1.5 text-[#c9a84c]"><Shield className="h-3.5 w-3.5" /> {profile.tier}</span>
                <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> Joined {format(new Date(profile.joined), "MMM yyyy")}</span>
              </div>
            </div>
            <div className="pb-2">
          ...
          <div className="grid md:grid-cols-3 gap-10 mt-12">
            ...
            <div className="md:col-span-1 space-y-8">
              <div>
                <h3 className="text-[10px] tracking-[0.4em] uppercase text-[#a8a8a0] mb-3">About the Tracker</h3>
              ...
              <div>
                <h3 className="text-[10px] tracking-[0.4em] uppercase text-[#a8a8a0] mb-3">Expedition Details</h3>
                <ul className="space-y-4 text-sm font-mono text-[#f5f5f0]">
                  <li className="flex flex-col gap-1 pb-3 border-b border-[#3d3d3d]">
                    <span className="text-[9px] uppercase tracking-widest text-[#5a5a55]">Primary Quarry</span>
                    <span>{profile.favorite_quarry}</span>
                  </li>
                  <li className="flex flex-col gap-1 pb-3 border-b border-[#3d3d3d]">
                    <span className="text-[9px] uppercase tracking-widest text-[#5a5a55]">Total Successful Hunts</span>
              ...
              {(profile.social?.instagram || profile.social?.website) && (
                <div>
                  <h3 className="text-[10px] tracking-[0.4em] uppercase text-[#a8a8a0] mb-3">External Links</h3>
                  <div className="space-y-2">
                    {profile.social.instagram && (
                      <a href={`https://instagram.com/${profile.social.instagram.replace('@', '')}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-[#c9a84c] hover:underline">
                        <ExternalLink className="h-3 w-3" /> Instagram ({profile.social.instagram})
                      </a>
                    )}
                    {profile.social.website && (
                      <a href={`https://${profile.social.website}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-[#c9a84c] hover:underline">
              ...
              {profile.trophies.length > 0 ? (
                <div className="grid sm:grid-cols-2 gap-4">
                  {profile.trophies.map((trophy: any) => (
                    <div key={trophy.id} className="bg-[#1a1a1a] border border-[#3d3d3d] p-5 hover:border-[#c9a84c]/30 transition group">
                      <div className="flex justify-between items-start mb-4">
                        <div className="text-3xl group-hover:scale-110 transition-transform origin-left">{trophy.emoji}</div>
                        <div className="text-xs font-mono text-[#5a5a55]">{format(new Date(trophy.date), "MMM yyyy")}</div>
                      </div>
                ...
                <div className="bg-[#1a1a1a] border border-[#3d3d3d] border-dashed p-10 text-center">
                  <p className="text-sm text-[#a8a8a0] font-serif italic">
                    This tracker hasn't published any completed expeditions to their public field record yet.
                  </p>
                </div>
              )}
            </div>
