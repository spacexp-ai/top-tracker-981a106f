  ...
  return (
    <div className="flex items-center gap-2 text-[#c9a84c]">
      ...
      <span className="font-display text-xl">
        {days}{" "}
        <span className="text-sm font-sans tracking-[0.2em] uppercase text-[#a8a8a0]">Days</span>
      </span>
    </div>
  );
}
...
function HuntWeather({ lat, lng }: { lat: number; lng: number }) {
  ...
  if (isLoading)
    return <div className="text-[10px] text-[#5a5a55] animate-pulse">Fetching Weather...</div>;
  ...
  return (
    <div className="flex items-center gap-3 text-xs text-[#a8a8a0]">
      <span className="flex items-center gap-1">
        <Thermometer className="h-3 w-3 text-[#c9a84c]" /> {temp}°F
      </span>
      <span className="flex items-center gap-1">
        <Cloud className="h-3 w-3 text-[#c9a84c]" /> Outlook
      </span>
    </div>
  );
}
        ...
        <div className="bg-[#2d2d2d] border border-[#3d3d3d] p-10 text-center animate-in fade-in relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            {tab === "upcoming" && (
              <Link
                to="/portal/book"
                className="mt-4 inline-block px-5 py-2.5 bg-[#c9a84c] hover:bg-[#b0903c] text-[#1a1a1a] text-[10px] tracking-[0.3em] uppercase transition"
              >
        ...
        <div className="grid gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {activeBookings.map((b) => {
            ...
            return (
              <div
                key={b.id}
                className="relative bg-[#2d2d2d] border border-[#3d3d3d] hover:border-[#c9a84c]/40 transition-all duration-300 group overflow-hidden flex flex-col md:flex-row shadow-sm hover:shadow-2xl hover:-translate-y-1"
              >
                ...
                <div className="flex-1 p-6 relative z-10">
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                    <div>
                      <div className="font-display text-2xl text-[#f5f5f0] group-hover:text-[#c9a84c] transition">
                        {b.species?.emoji} {b.species?.name ?? "Draft expedition"}
                      </div>
                ...
                <div className="relative z-10 bg-[#1a1a1a]/80 p-4 md:w-48 flex flex-row md:flex-col gap-2 justify-center border-t md:border-t-0 md:border-l border-[#3d3d3d] backdrop-blur-sm">
                  {isDraft ? (
                    <Link
                      to="/portal/book"
                      search={{ id: b.id }}
                      className="w-full inline-flex items-center justify-center gap-2 px-4 py-2 bg-[#c9a84c] text-[#1a1a1a] text-[10px] tracking-[0.2em] uppercase hover:bg-[#b0903c] transition"
                    >
                    ...
                    <>
                      <button
                        ...
                        className="flex-1 md:w-full inline-flex items-center gap-2 px-3 py-2 border border-[#3d3d3d] hover:border-[#c9a84c] text-[10px] tracking-[0.2em] uppercase text-[#a8a8a0] hover:text-[#c9a84c] transition bg-[#2d2d2d]"
                      >
      ...
      {showKitFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-[#1a1a1a] border border-[#3d3d3d] max-w-lg w-full p-6 relative">
            <button
              onClick={() => setShowKitFor(null)}
              className="absolute top-4 right-4 text-[#a8a8a0] hover:text-[#c9a84c]"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              ...
              <ul className="space-y-2">
                {[
                  ...
                  <li
                    key={i}
                    className="flex items-start gap-3 p-3 bg-[#2d2d2d] border border-[#3d3d3d]"
                  >
