    ...
    <div className="grid lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-6">
        <div className="bg-card p-6 border border-border">
          <h3 className="font-display text-lg tracking-[0.1em] uppercase text-forest">
            ...
            <div className="mt-3 flex gap-2">
              {[1, 2, 3, 4].map((n) => (
                <button
                  ...
                  className={`flex-1 py-2 border tracking-widest text-sm transition ${hunters === n ? "border-ember bg-ember text-bone" : "border-border hover:border-accent"}`}
                >
                  {n}
                </button>
              ))}
        ...
        <div className="bg-card p-6 border border-border">
          <h3 className="font-display text-base tracking-[0.1em] uppercase text-forest">
            ...
            {species.map((s) => {
              ...
              return (
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  onClick={() => toggle(s.id)}
                  className={`flex justify-between items-center p-3 border transition ${on ? "border-ember bg-ember/10" : "border-border hover:border-accent"}`}
                >
                  <span className="font-serif text-base text-forest">
                    {t("estimator_embed.species." + s.id, s.label)}
                  </span>
                  <span
                    className={`font-display text-xs ${on ? "text-ember" : "text-muted-foreground"}`}
                  >
                    {fmt(s.fee)}
                  </span>
                </motion.button>
          ...
          <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-2">
            {tiers.map((opt) => (
              <motion.button
                ...
                className={`p-3 text-left border transition ${tier === opt.id ? "border-ember bg-ember/10" : "border-border hover:border-accent"}`}
              >
                <div className="font-display text-sm text-forest">
      ...
      <div className="lg:sticky lg:top-4 lg:self-start">
        <div className="bg-ink text-bone p-6 border border-accent/40">
          <div className="text-[10px] tracking-[0.4em] uppercase text-accent">
            {t("estimator_embed.live_estimate", "Live estimate")}
          </div>
