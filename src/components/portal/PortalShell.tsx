    ...
    <div className="relative min-h-screen bg-[#1a1a1a] text-[#f5f5f0]">
      ...
      <header className="relative z-40 sticky top-0 bg-[#1a1a1a]/90 backdrop-blur border-b border-[#3d3d3d] px-4 md:px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <Link to="/portal" className="font-display text-[#c9a84c] text-lg tracking-wider">
            ◆ TOP TRACKERS
          </Link>
        </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-3 hover:bg-[#2d2d2d] py-1 px-2 rounded-md transition-colors outline-none focus:ring-1 focus:ring-[#c9a84c]">
              ...
              <Avatar className="h-8 w-8 border border-[#3d3d3d]">
                <AvatarImage
                  src={
                    data?.profile?.avatar_url || "https://i.pravatar.cc/150?u=a042581f4e29026024d"
                  }
                  alt="@tracker"
                />
          ...
          <DropdownMenuContent
            className="w-56 bg-[#2d2d2d] border-[#3d3d3d] text-[#f5f5f0]"
            align="end"
            forceMount
          >
            ...
            <DropdownMenuGroup>
              <DropdownMenuItem
                asChild
                className="focus:bg-[#1a1a1a] focus:text-[#c9a84c] cursor-pointer"
              >
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside
          className={`${open ? "block" : "hidden"} lg:block fixed lg:sticky top-[65px] lg:top-[65px] z-30 left-0 h-[calc(100vh-65px)] w-64 bg-[#2d2d2d] border-r border-[#3d3d3d] overflow-y-auto`}
        >
          <nav className="p-4 space-y-1">
            {nav.map((n) => (
              <Link
                ...
                activeProps={{
                  className:
                    "flex items-center gap-3 px-3 py-2.5 text-sm text-[#c9a84c] bg-[#1a1a1a] border-l-2 border-[#c9a84c]",
                }}
              >
            ...
            <div className="pt-6 pb-2 px-3 text-[10px] tracking-[0.3em] uppercase text-[#5a5a55]">
              Coming soon
            </div>
            {soon.map((s) => (
              <div
                key={s.label}
                className="flex items-center justify-between gap-3 px-3 py-2.5 text-sm text-[#5a5a55]"
              >
                <span className="flex items-center gap-3">
                  <s.icon className="h-4 w-4" /> {s.label}
                </span>
                <span className="text-[9px] tracking-[0.2em] uppercase text-[#c9a84c]/60">
                  Soon
                </span>
              </div>
            ))}
